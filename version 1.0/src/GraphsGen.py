...
def createLollipopPlot(data, filepath):
    ...
    for x, y, tex in zip(ordered_df['endTime'], my_range, ordered_df['endNode']):
        plt.text(x+1, y, simplifyText(tex), horizontalalignment='left', verticalalignment='center', fontdict={'color': 'black', 'size': 10})

    #added for different color hline
    for x, y, z, tex in zip(ordered_df['startTime'], my_range, ordered_df['endTime'], ordered_df['compBy']):
        if str(tex) == "attacker-1":
            plt.hlines(y=y, xmin=x, xmax=z, color=colors[0], alpha=1)
        elif str(tex) == "attacker-2":
            plt.hlines(y=y, xmin=x, xmax=z, color=colors[1], alpha=1)
        elif str(tex) == "attacker-3":
            plt.hlines(y=y, xmin=x, xmax=z, color=colors[2], alpha=1)
        elif str(tex) == "attacker-4":
            plt.hlines(y=y, xmin=x, xmax=z, color=colors[3], alpha=1)
        else:
            plt.hlines(y=y, xmin=x, xmax=z, color=colors[4], alpha=1)
    
    #for dummy legend
    templist = list(dict.fromkeys(ordered_df['compBy']))
    for x in templist:
        if str(x) == "attacker-1":
            plt.hlines(y=0, xmin=0, xmax=0, color=colors[0], alpha=1, label=str(x))
        elif str(x) == "attacker-2":
            plt.hlines(y=0, xmin=0, xmax=0, color=colors[1], alpha=1, label=str(x))
        elif str(x) == "attacker-3":
            plt.hlines(y=0, xmin=0, xmax=0, color=colors[2], alpha=1, label=str(x))
        elif str(x) == "attacker-4":
            plt.hlines(y=0, xmin=0, xmax=0, color=colors[3], alpha=1, label=str(x))
        else:
            plt.hlines(y=0, xmin=0, xmax=0, color=colors[4], alpha=1, label=str(x))
...
def createGraph(net, filename, filepath):
    ...
    pos1 = [[0, 1000], [500, 1000], [1000, 1000], [1500, 1000], [2000, 1000], [2500, 1000]]
    ...
    for node in net.nodes:
        ...
        if len(node.position) > 0:
            ...
            p = pos1[i]
            nodeSizeList.append(150)
            i += 1

        if 'attacker' in text or 'ag_attacker' in text:
            if '1' in text:
                g.add_node(node.name, color="darkred", size=150, shape="circle", pos=p)
            elif '2' in text:
                g.add_node(node.name, color="darkcyan", size=150, shape="circle", pos=p)
            elif '3' in text:
                g.add_node(node.name, color="olive", size=150, shape="circle", pos=p)
            elif '4' in text:
                g.add_node(node.name, color="gold", size=150, shape="circle", pos=p)
            elif '5' in text:
                g.add_node(node.name, color="purple", size=150, shape="circle", pos=p)
            else:
                g.add_node(node.name, color="hotpink", size=150, shape="circle", pos=p)
        elif 'CNC' in text or 'ag_CNC' in text:
            g.add_node(node.name, color="orange", size=150, shape="circle", pos=p)
        else:
            if len(node.log) > 0:
                temp = node.log[1]
            if temp is not None:
                if temp == "attacker-1":
                    g.add_node(node.name, size=nodeSize, color="red", pos=p)
                elif temp == "attacker-2":
                    g.add_node(node.name, size=nodeSize, color="aqua", pos=p)
                elif temp == "attacker-3":
                    g.add_node(node.name, size=nodeSize, color="green", pos=p)
                elif temp == "attacker-4":
                    g.add_node(node.name, size=nodeSize, color="yellow", pos=p)
                elif temp == "attacker-5":
                    g.add_node(node.name, size=nodeSize, color="mediumpurple", pos=p)
                else:
                    g.add_node(node.name, size=nodeSize, color="pink", pos=p)
            else:
        ...
        for conNode in node.con:
            ...
            if 'attacker' in text2 or 'ag_attacker' in text2:
                if '1' in text2:
                    g.add_node(conNode.name)#, color="darkred", size=50, shape="circle")#, pos=p2)
                elif '2' in text2:
                    g.add_node(conNode.name)#, color="darkcyan", size=50, shape="circle")#, pos=p2)
                elif '3' in text2:
                    g.add_node(conNode.name)#, color="olive", size=50, shape="circle")#, pos=p2)
                elif '4' in text2:
                    g.add_node(conNode.name)#, color="gold", size=50, shape="circle")#, pos=p2)
                elif '5' in text2:
                    g.add_node(conNode.name)#, color="purple", size=50, shape="circle")#, pos=p2)
                else:
                    g.add_node(conNode.name)#, color="hotpink", size=50, shape="circle")#, pos=p2)
            elif 'CNC' in text2 or 'ag_CNC' in text2:
                g.add_node(conNode.name)#, color="orange", size=50, shape="circle")#, pos=p2)
            else:
                ...
                if temp2 is not None:
                    if temp2 == "attacker-1":
                        g.add_node(conNode.name)#, size=nodeSize2, color="red")#, pos=p2)
                    elif temp2 == "attacker-2":
                        g.add_node(conNode.name)#, size=nodeSize2, color="aqua")#, pos=p2)
                    elif temp2 == "attacker-3":
                        g.add_node(conNode.name)#, size=nodeSize2, color="green")#, pos=p2)
                    elif temp2 == "attacker-4":
                        g.add_node(conNode.name)#, size=nodeSize2, color="yellow")#, pos=p2)
                    elif temp2 == "attacker-5":
                        g.add_node(conNode.name)#, size=nodeSize2, color="mediumpurple")#, pos=p2)
                    else:
                        g.add_node(conNode.name)#, size=nodeSize2, color="pink")#, pos=p2)
                else:
                    g.add_node(conNode.name)#, size=nodeSize2, color="gray")
            ...
            if len(conNode.log) > 0 and ('CNC' in text or 'ag_CNC' in text):
                ...
                if temp3 is not None:
                    if temp3 == "attacker-1":
                        g.add_edge(conNode.name, node.name, weight=100, color='red', style='dashed')
                    elif temp3 == "attacker-2":
                        g.add_edge(node.name, conNode.name, weight=100, color='aqua', style='dashed')
                    elif temp3 == "attacker-3":
                        g.add_edge(node.name, conNode.name, weight=100, color='green', style='dashed')
                    elif temp3 == "attacker-4":
                        g.add_edge(node.name, conNode.name, weight=100, color='yellow', style='dashed')
                    elif temp3 == "attacker-5":
                        g.add_edge(node.name, conNode.name, weight=100, color='mediumpurple', style='dashed')
                    else:
                        g.add_edge(node.name, conNode.name, weight=100, color='pink', style='dashed')
            else:
    ...
    if len(nameMapping) > 0:
        ...
        nx.draw(g, pos, node_size=ns, node_color=colors1, edge_color=edge_color_list, style='dashed', with_labels=True, font_size=12)
