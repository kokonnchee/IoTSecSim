...
#=================================================================================================
# Conventional Defence Techniques Modelling
#=================================================================================================
    ...
    def antiMalwareSystem(self, mode, checkLocation, node, content):
        """
        Detects and blocks malicious traffic
        Cleanse the compromised node

        Behavioural Analysis - later version - pending
        Signature-based detection - basic version
        cloud-based scanning - advanced version - pending
        sandbox feature - advanced version - pending
        ...
        if mode == "simple1" or mode == "simple2":
            if len(checkLocation) > 0:
                for x in checkLocation:
                    if x == "memory":
                        uniqueList = list(dict.fromkeys(node.processlist))
                        for y in uniqueList:
                            if node.processlist.count(y) > 1:
                                suspicionNum += 1

                    if x == "folder":
                        if len(node.folderlist) > 0:
                            for z in node.folderlist:
                                if z == "cron":
                                    uniqueList = list(dict.fromkeys(node.cronFolder))
                                    for y in uniqueList:
                                        if node.cronFolder.count(y) > 1:
                                            suspicionNum += 1
                                if z == "init":
                                    uniqueList = list(dict.fromkeys(node.initFolder))
                                    for y in uniqueList:
                                        if node.initFolder.count(y) > 1:
                                            suspicionNum += 1
                    if x == "file":
                        uniqueList = list(dict.fromkeys(node.filelist))
                        for y in uniqueList:
                            if node.filelist.count(y) > 1:
                                suspicionNum += 1
            else:
                print("Check location not found.")
        elif mode == "basic":
            if len(checkLocation) > 0:
                for x in checkLocation:
                    if x == "memory":
            ...
            print("Anti-malware version not found.")

        if suspicionNum > 0:
            suspicionDetected = True

        return malwareDetected, suspicionDetected, suspicionNum
...
#=================================================================================================
# Advanced Defence Techniques Modelling
#=================================================================================================

    def movingTargetDefence(self, net, mode, times, nodeNum, shufflelist, isolationlist, resetlist, restorelist):
        """
        ...
        """
        probability = 0.7
        if mode == 1: #"topology shuffling":
            net = self.randomShufflingNew(net, probability, times)
        elif mode == 2: #"IP shuffling": - pending
            pass
        elif mode == 3: #"AP shuffling":
            net = self.shufflingAP(net, nodeNum, shufflelist, []) #isolationlist)
        elif mode == 4: #"isolation"
            for x in isolationlist:
                for node in net.nodes:
                    if node.name == x:
                        templist = self.nodeIsolation2(node)
                        if x not in resetlist:
                            resetlist.append(x)
                        if templist not in restorelist:
                            restorelist.append(templist)
        elif mode == 5: #"reset"
            for x in resetlist:
                for node in net.nodes:
                    if node.name == x:
            ...
            for x in restorelist:
                #format: [[node, nodelist]]
                self.restoreConnection(x[0], x[1])

        else:
            print("MTD mode not found!")

        return net
    
    def shufflingAP(self, net, nodeNum, shufflelist, isolationlist):
        """
        ...
        if len(isolationlist) > 0:
            for x in net.nodes:
                if x.name in isolationlist:
                    tempList.append(x)
        else:
            while(nodeNum > 0):
                temp1 = choice(net.nodes)
                if len(temp1.con) > 0:
                    text = temp1.name.split('-')
                    text2 = temp1.name.split("+")
                    text3 = text[0].split('ag_')
                    if 'attacker' in text or 'ag_attacker' in text or 'CNC' in text or 'ag_CNC' in text or 'server' in text or 'ag_server' in text or 'router' in text or 'ag_router' in text or 'Intelligence Center' in text or 'SDN switch' in text:
                        pass
                    elif 'ag_decoy' in text2 or 'decoy' in text2:
                        pass
                    elif temp1 in tempList:
                        pass
                    else:
                        if len(shufflelist) > 0:
                            if text3[-1] in shufflelist:
        ...
        if len(isolationlist) > 0:
            ...
            for node in tempList:
                ...
                for x in node.con:
                    text = x.name.split('-')
                    if 'router' in text or 'ag_router' in text:
                        tempAP1.append(x)
                        for y in x.con:
                            text2 = y.name.split('-')
                            if 'router' in text2 or 'ag_router' in text2:
                                tempAP2.append(y)

                if len(tempAP1) > 0 and len(tempAP2) > 0:
                    temp1 = choice(tempAP1)
                    disconnectTwoWays(node, temp1)

                    if node.lastAP == None:
                        temp2 = choice(tempAP2)
                        connectTwoWays(node, temp2)
                    else:
                        for z in tempAP2:
                            if z.name == node.lastAP:
                                connectTwoWays(node, z)
                    node.lastAP = temp1.name
    ...
    def randomShufflingNew(self, net, probability, times):
        ...
        """
        shuffledNet = copyNet(net)# danger when nodes num > 700
    ...
    def addDecoyNode(self, net, model, mode):
        ...
        #randomly select a node to duplicate
        while (tempTotal > 0):
            x = choice(net.nodes)
            if x in tempList:
                pass
        ...
        for i in range(total):
            dnode = realNode("decoy+"+tempList[i].name)
            dnode = copy.deepcopy(tempList[i])
            dnode.name = "decoy+"+tempList[i].name
            
            dnode.decoy = True
            dnode.type = False #"emulated" # might wana change this
            dnode.model = choice(modelList)

            if mode == False:
                temp = list(tempList[i].position)
                tempnum = []
                tempnum.append(temp[0]+100)
                tempnum.append(temp[1]+100)
                dnode.position = tuple(tempnum)
    ...
    def decoyReports(self, content, content2, filepath, response, node):
        """
        Make a decoy report 
        """
        ...
        if response != None and action == "Unauthorised Accessing":
            nodeReset = True
    ...
    def reset(self, node):
        """
        Reset node status without patching node's vulnerability
        """
        #deep clean infected node
        for y in node.realPort:
            if node.realPort[y]['open'] == False:
    ...
    def nodeIsolation(self, node, intelNode):
        """
        Isolate infected IoT node
        """
        for x in node.con:
            if x != intelNode:
                if type(x) != attacker and type(x) != CommandNControl:
                    disconnectTwoWays(node, x)

        return None
    
    def nodeIsolation2(self, node):
        """
        Isolate infected IoT node
        """
        restoreConList = []
        if type(node) != attacker and type(node) != CommandNControl:
            for x in node.con:
                if type(x) != attacker and type(x) != CommandNControl:
                    disconnectTwoWays(node, x)
                    restoreConList.append(x)

        return [node, restoreConList]
    
    def restoreConnection(self, node, resConList):
        """
        Restore connection to cleansed IoT node
        """
        for x in resConList:
            connectTwoWays(node, x)
