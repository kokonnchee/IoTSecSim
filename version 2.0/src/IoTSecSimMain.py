...
def initialiseIoTNetwork(mode, IoTDeviceSetup, topologyStyle, attackerList, IoTDefence, AtkerTimeDataDict, saveSimDir, saveFolder, entryPointNode, percentageOfVulnNodes, specialChanges, graphDensity, wlg):
    device = []
    iotdeviceNum = 0
    lightbulbNum = 0
    slowcookerNum = 0
    ...
    net = createIoTNetwork(device, topologyStyle, IoTDeviceSetup, IoTDefence, AtkerTimeDataDict, saveSimDir, saveFolder, entryPointNode, percentageOfVulnNodes, specialChanges, graphDensity, wlg) ## for other topologies

    ddosAttack = False
    net, ddosAttack = addAttackersToPool(net, attackerList)
    if ddosAttack == True:
        net = addDDoSTargetServersToPool(net, 5)
...
def runSimulation(inputfile, simRunName, currentDir):
    ...
    if topologyStyle == "scalable":
        repeatNum = 1 #for scalability testing

    folderName = "none"
    wlg = wordlistGen()
    
    entryPointNode = []#specific entry point for attacker, e.g., "iotdevice-1", "iotdevice-2", "iotdevice-3", "router", etc.
    ...
    for x in IoTAttack:
        ...
        if IoTAttack[x]["exploitType"][0] == 'dc':# or IoTAttack[x]["exploitType"][0] == 'common' or IoTAttack[x]["exploitType"][0] == 'strong':
            IoTAttack[x]["credentialList"] = wlg.attackerWordList(IoTAttack[x]["exploitType"][1], IoTAttack[x]["exploitType"][2])
        elif IoTAttack[x]["exploitType"][0] == 'vuln':
            if IoTAttack[x]["exploitType"][3] != 'nothing':
                IoTAttack[x]["exploit"].append(IoTAttack[x]["exploitType"][3])
        elif IoTAttack[x]["exploitType"][0] == 'mixed-v':
            IoTAttack[x]["credentialList"] = wlg.attackerWordList(IoTAttack[x]["exploitType"][1], IoTAttack[x]["exploitType"][2])
            if IoTAttack[x]["exploitType"][3] != 'nothing':
                IoTAttack[x]["exploit"].append(IoTAttack[x]["exploitType"][3])
        else:
    ...
    removeCategory = []
    for x in AtkerTimeDataDict:
        for y in AtkerTimeDataDict[x]:
            if y != "AverageTime":
                for z in AtkerTimeDataDict[x][y]:
                    if z == "proportion":
                        if AtkerTimeDataDict[x][y][z] == 0:
                            temp = []
                            temp.append(str(x))
                            temp.append(str(y))
                            removeCategory.append(temp)
    for x in removeCategory:
        AtkerTimeDataDict[x[0]].pop(x[1])
    ...
    for i in range(rank, repeatNum, num_procs): 
        ...
        officeDifSim = initialiseIoTNetwork("office", IoTDeviceSetupcopy, topologyStyle, iotatkcopy, IoTDefencecopy, AtkerTimeDataDictcopy, saveSimDir, saveFolder, entryPointNode, percentageOfVulnNodes, specialChanges, graphDensity, wlg) ## False for different nodes
