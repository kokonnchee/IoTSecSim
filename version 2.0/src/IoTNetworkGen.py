...
def addScalableTopology(net, router, topoType, saveFolder):
    """
    Create a graph for scalability testing
    """
    num = len(net.nodes)
    
    if topoType != None:
        data, pos = createScalableNetwork(num, saveFolder, topoType)
    else:
        print("Topology style not found!!")
    ...
    while (tempNum2 > 0):
        ...
        tempNodeCon = tempNode.con.copy()
        tempRouterCon = router[i].con.copy()

        templist1 = []
        for x in tempNodeCon:
            if x not in router[i].con:
                templist1.append(x)
                connectTwoWays(router[i], x)
                disconnectTwoWays(tempNode, x)
        router[i].con = templist1.copy()

        templist2 = []
        for x in tempRouterCon:
            if x not in tempNode.con:
                templist2.append(x)
                connectTwoWays(tempNode, x)
                disconnectTwoWays(router[i], x)
        tempNode.con = templist2.copy()
...
def addDefaultCredentialToDevice(net, IoTDeviceSetup, wlg):
    """
    Add credential protection to real devices.
    """

    for x in IoTDeviceSetup:
        u = ""
        p = ""
        if IoTDeviceSetup[x]["defaultCredential"][3] == "identical":
            u, p = wlg.setupCredentialForDevice(IoTDeviceSetup[x]["defaultCredential"][4])
            for node in net.nodes:
                text = node.name.split("-")
                if str(text[0]) == str(x):
                    node.loginUsername = u
                    node.loginPassword = p
                    node.credentialPort.append(IoTDeviceSetup[x]['defaultCredential'][0])
                    if IoTDeviceSetup[x]['defaultCredential'][2] == "True":
                        node.credentialPort.append(True)
                    else:
                        node.credentialPort.append(False)
        else:
            for node in net.nodes:
                text = node.name.split("-")
                if str(text[0]) == str(x):
                    node.loginUsername, node.loginPassword = wlg.setupCredentialForDevice(IoTDeviceSetup[x]["defaultCredential"][4])
                    node.credentialPort.append(IoTDeviceSetup[x]['defaultCredential'][0])
                    if IoTDeviceSetup[x]['defaultCredential'][2] == "True":
                        node.credentialPort.append(True)
                    else:
...
def addOtherDefaultSettingsToDevice(net, IoTDeviceSetup):
    """
    Add other configurations to real devices.
    """
    for x in IoTDeviceSetup:
        for node in net.nodes:
            text = node.name.split("-")
            if str(text[0]) == str(x):
                node.resourceMeterAll = IoTDeviceSetup[x]["resourceConsumptionNLimit"][1]
                node.resourceMeterCurrent = IoTDeviceSetup[x]["resourceConsumptionNLimit"][0]
                node.resourceMeterBreakLimit = 95
                node.resourceConsumptionNLimit = IoTDeviceSetup[x]["resourceConsumptionNLimit"]
                
                node.filelist = IoTDeviceSetup[x]["filelist"].copy()
                node.folderlist = IoTDeviceSetup[x]["folderlist"].copy()
                node.cronFolder = IoTDeviceSetup[x]["cronFolder"].copy()
                node.initFolder = IoTDeviceSetup[x]["initFolder"].copy()
                node.processlist = IoTDeviceSetup[x]["processList"].copy()

                node.conditionChoice = ["enable", "disable", "rebooting", "crashed", "busy"]
                node.isRebooting = False

                node.rebootable = IoTDeviceSetup[x]["rebootable"]
                node.conditionNow = IoTDeviceSetup[x]["condition"]

    return None

def createIoTNetwork(node_vlan_list, topologyStyle, IoTDeviceSetup, defMode, AtkerTimeDataDict, saveDir, saveFolder, entryPointNode, percentageOfVulnNodes, specialChanges, graphDensity, wlg): ## add num for random graph
    """
    ...
    elif topologyStyle == "scalable":
        addScalableTopology(net, routersNum, "RGG", saveFolder) #"grid"
    else:
        print("Topology style not found!")
        sys.exit(1)
    ...
    addDefaultCredentialToDevice(net, IoTDeviceSetup, wlg)
    addOtherDefaultSettingsToDevice(net, IoTDeviceSetup)
    ...
    # asfilename = str(folderName[-1])+"+AtkSurface.t"

    # if os.path.isfile(asfilename):
    #     pass
    # else:
    #     initialAS = calculateAttackSurface(net, 0, None, None)
    ...
    if cdStatus == True:
        ...
        if newNet != None:
            net = copyNet(newNet)# danger when nodes num > 700



    if defMode["Firewall"]["operational"] == True:
        for z in net.defMode["Firewall"]["rule"]:
            if net.defMode["Firewall"]["rule"][z] != None:
    ...
    if defMode["IPS"]["operational"] == True:
        for z in net.defMode["IPS"]["rule"]:
            if net.defMode["IPS"]["rule"][z] != None:
    ...
    if defMode["IDS"]["operational"] == True:
        for z in net.defMode["IDS"]["rule"]:
            if net.defMode["IDS"]["rule"][z] != None:
    ...
    if defMode["MTD"]["operational"] == True:
        sdnNode = sdnSwitch('SDN switch') # how to setup multi intel center
        sdnNode.canBeCompromised = False
        net.nodes.append(sdnNode)

    # if os.path.isfile(asfilename):
    #     pass
    # else:
    #     currentAS = calculateAttackSurface(net, initialAS, saveFolder, asfilename)
...
def addDDoSTargetServersToPool(net, num):
    """
    Add target servers to network for ddos attacks.
    """ 
    ...
    for i in range(num):
        server = None
        server = Server('server-' + str(i+1))
        server.IPv4Add = assignRandomIPAddress(4)
        net.nodes.append(server)

    for x in net.nodes:
        text = x.name.split("-")
        if 'server' in text:
            print(x.name, x.IPv4Add)

    return net

def addAttackersToPool(net, attackerList):
    """
    Add attacker to network.
    """ 
    i = 1
    ...
    for id, info in attackerList.items():
        ...
        for key in info:
            if key == 'AverageTime':
                A.meanTime = info[key]
            elif key == 'propagationType':
                A.propagationType = info[key]
            elif key == 'scanPort':
                A.scanPort = info[key]
            elif key == 'exploitType':
                A.exploitType = info[key]
            elif key == 'IP':
                A.IPv4Add = info[key]
            elif key == 'protocol':
                A.protocol = info[key]
                ...
                A.botCollude = info[key]
            elif key == 'group':
                A.group = info[key]
            elif key == 'binaryName':
                A.binaryName = info[key]
            elif key == 'goal':
                A.goal = info[key]
            elif key == 'mode':
                A.mode = info[key]
            ...
            elif key == 'respondToReboot':
                A.respondToReboot = info[key]
            elif key == 'resideLocation':
                A.resideLocation = info[key]
            elif key == 'botTaskList':
                A.botTaskList = info[key]
            elif key == 'botActionList':
                A.botActionList = info[key]
            elif key == 'killerBlackList':
                A.killerBlackList = info[key]
            elif key == 'fortificationList':
                A.fortificationList = info[key]
            elif key == 'evasionList':
                A.evasionList = info[key]
            elif key == 'credentialList':
                A.carryCredential = info[key]
            else:
                print("Error")
        A.active = True
        ...
        if A.goal == "all":
            ...
            for x in net.nodes:
                if x.canBeCompromised == True:
                    tempNum += 1
            A.goal = int(tempNum)
            attackerList[id]['goal'] = int(tempNum)

        for x in A.botTaskList:
            if x[0] == "ddos":
                ddosAttack = True

        #trying to add same CNC to collude and same group attackers and vice versa
        if CNC == None:
            temp = []
            if A.collude == True:
            ...
            CNC.binaryName = A.binaryName.copy()
            
        else:
            pass
        CNC.setIdle()
        CNC.resideLocation = A.resideLocation.copy()
        CNC.botTaskList = A.botTaskList.copy()
        CNC.respondToReboot = A.respondToReboot
        CNC.botActionList = A.botActionList.copy()
        connectTwoWays(CNC, A)
        A.CNCNode.append(CNC.name)

        binaryfile = binaryFile(A.binaryName[0])
        binaryfile.filesize = randint(100, 300)
        A.binaryfile.append(binaryfile)
        CNC.binaryfile.append(binaryfile)
